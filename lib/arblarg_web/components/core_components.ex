defmodule ArblargWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: ArblargWeb.Gettext
  ...
  def flash_group(assigns) do
    ~H"""
    <div class="fixed top-0 right-0 z-50 space-y-4 p-4 sm:p-6">
  ...
  attr :rest, :global, include: ~w(disabled form name value size)
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-zinc-800 hover:bg-zinc-700",
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "block w-full rounded-lg text-white bg-zinc-900 border border-zinc-700",
          "phx-no-feedback:border-zinc-700 phx-no-feedback:focus:border-zinc-600",
          "px-3 py-2 text-sm leading-6",
          "font-mono",
          @errors == [] && "focus:border-zinc-600",
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-white font-mono">
      {render_slot(@inner_block)}
    </label>
    """
  end
  ...
  def error(assigns) do
    ~H"""
    <p class="mt-3 flex gap-3 text-sm leading-6 text-rose-400 font-mono">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ArblargWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ArblargWeb.Gettext, "errors", msg, opts)
    end
  end
